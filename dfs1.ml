...
* So just use the function of exists in the List to test
*)
let is_ending outedges to_id =  
  List.exists (fun (label, id) -> id = to_id) outedges 
...
(*
* define the function of getting the index of specific vertex
* when use it, the acu should be 0, which can directly used in getting element
*)
let rec get_index outedges to_id acu =
  match outedges with
  | [] -> -1
  | (label, id) :: rest -> 
    if id = to_id then acu
    else get_index rest to_id acu+1
...
(*
* define the function of judging the stack should pop the top vertex or not  
*)
(*  
let should_pop vertex_info_stack =
  let (id, position, length) = Stack.top vertex_info_stack in
  if position = length-1 then true else false
*) 
...
let get_path_from_stack graph vertex_stack =
  let rec loop graph stack path_acu = 
    ...
    if Stack.empty stack then path_acu
    else
      let (id_avant,_,_) = Stack.top stack in
      let vertex_info_avant = Graph.find_vertex graph id_avant in
      let label = get_label_by_id vertex_info_avant id_top in
      loop graph stack (id_avant,label, id_top)::path_acu
...
let rec get_path graph vertex_stack to_id =
  ...
  then 
    ...
    if Stack.empty vertex_stack then []
    ...
    then 
      ...
      let outedges_next = vertex_next_info in
...
*)
  ...
  *)
        ...
        let vertex_info_from = Graph.find_vertex from_id in
